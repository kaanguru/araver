...
const pauseIconUrls = ["icons/icons8_pause_64px.png", "icons8_Body_Positive_Female_64px_1.png", "jumping-man.png"];
...
class PauseScreen {
  ...
  onStateChanged(newState: string): void {
    ...
    if (newState != "active") {
      ...
      if (minutes >= pauseReminder.settings.breakDuration - 1) {
        ...
        console.log(`${timeForLogging}: away for  ${(diff / (1000 * 60)).toString()}m reseting break timer`);
  ...
  onAlarmFired(alarm: chrome.alarms.Alarm): void {
    ...
    if (pauseReminder.timeWithoutBreak >= pauseReminder.settings.breakInterval) {
      ...
      chrome.notifications.create("pause", { type: "basic", title: breakTitle, iconUrl: "icons/icons8_stretching_64px.png", message: breakContent(pauseReminder.settings.breakDuration.toString()) });
    } else {
      console.log(`${timeForLogging}:  Short Pause, without break since  ${pauseReminder.timeWithoutBreak.toString()} minutes`);
      let randomString = pauseIconUrls[Math.floor(Math.random() * pauseIconUrls.length)];

      console.log(randomString);
      chrome.notifications.create("pause", { type: "basic", title: reminderTitle, iconUrl: pauseIconUrls[0], message: reminderContent });
    }
  }
  loadSettings(): void {
    const settings = {
